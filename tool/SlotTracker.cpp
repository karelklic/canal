...
SlotTracker::SlotTracker(const llvm::Module *module)
    : mModule(module), mFunction(0), mFunctionProcessed(false),
...
int
SlotTracker::getGlobalSlot(const llvm::GlobalValue *value)
{
    // Check for uninitialized state and do lazy initialization.
    initialize();
...
{
    CANAL_ASSERT(value && "Can't insert a null Value into SlotTracker!");
...
{
    CANAL_ASSERT(node && "Can't insert a null Value into SlotTracker!");
