...
Types & \textit{typ} & ::= & \textbf{i}\textit{sz}~~|~~\textit{fp}~~|~~\textbf{void}~~|~~\textit{typ}$*$~~|~~[ \textit{sz} $\times$ \textit{typ} ]~~|~~$\big\{\overline{\textit{typ}_j}^j\big\}$~~|~~\textit{typ} $\overline{\textit{typ}_j}^j$~~|~~\textit{id}~~|~~\textbf{label} \\
& & | & \textbf{metadata} \\
