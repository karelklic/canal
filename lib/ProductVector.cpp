...
Vector::operator==(const Domain &value) const
{
    ...
    const Vector *container = llvm::dyn_cast<Vector>(&value);
    if (!container)
        return false;
...
{
    std::vector<Domain*>::iterator it = mValues.begin();
    const Vector &container = llvm::cast<Vector>(value);
    CANAL_ASSERT(mValues.size() == container.mValues.size());
...
{
    const Vector &aa = llvm::cast<Vector>(a),
        &bb = llvm::cast<Vector>(b);
...
Vector::insertelement(const Domain &array,
                         ...
                         const Domain &index)
{
    const Vector &container = llvm::cast<Vector>(array);
...
{
    const Vector &container = llvm::cast<Vector>(aggregate);
    CANAL_ASSERT(container.mValues.size() == mValues.size());
